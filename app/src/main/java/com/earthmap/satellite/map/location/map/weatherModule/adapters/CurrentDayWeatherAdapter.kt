...
import com.earthmap.satellite.map.location.map.databinding.WeatherTodayItemStyleBinding
...
):RecyclerView.Adapter<CurrentDayWeatherAdapter.CurrentDayWeatherViewHolder>() {


    class CurrentDayWeatherViewHolder(biningitemView: WeatherTodayItemStyleBinding) : RecyclerView.ViewHolder(biningitemView.root) {
        var itemImg: ImageView? = null
        var DayTxt: TextView? = null
        var Loading: ProgressBar? = null
        var TempTxt: TextView? = null
        init {
            itemImg=biningitemView.CrntItemImg
            DayTxt=biningitemView.CrntItemTimeTxt
            Loading=biningitemView.CrntItemProgress
            TempTxt=biningitemView.CrntItemTempTxt
        }

    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CurrentDayWeatherViewHolder {
        val binding =
            WeatherTodayItemStyleBinding.inflate(LayoutInflater.from(mContext), parent, false)
        return CurrentDayWeatherViewHolder(binding)
    }

    override fun onBindViewHolder(holder: CurrentDayWeatherViewHolder, position: Int) {
...
//            holder.DayTxt!!.setText(goal2)
    ...
    private fun getWeatherConditions(cond: String, holder: CurrentDayWeatherAdapter.CurrentDayWeatherViewHolder) {
        when (cond) {
            "04d" ->
                try {
