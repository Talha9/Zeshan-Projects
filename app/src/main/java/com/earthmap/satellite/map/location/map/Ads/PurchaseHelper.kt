...
class PurchaseHelper(private val activityContext: Context) :
    PurchasesUpdatedListener {

    private lateinit var googleBilling: BillingClient
    private val TAG = "BillingLogger:"
    private val listAvailPurchases = ArrayList<SkuDetails>()
    private lateinit var billingPreferences: SharedPreferences
    ...
    private fun initMyBillingClientLiveStreetView() {
        ...
        googleBilling = BillingClient
            .newBuilder(activityContext)
            .enablePendingPurchases()
            .setListener(this)
            .build()
        googleBilling.startConnection(object : BillingClientStateListener {
            override fun onBillingSetupFinished(billingResult: BillingResult) {
                if (billingResult.responseCode == BillingClient.BillingResponseCode.OK) {
                    Log.d(TAG, "Google Billing is Connected")
                    fetchAllInAppsFromConsole() /*available on console*/
                    fetchPurchasedInAppsFromConsole()  /*user has purchased*/
                }
            }
    ...
    private fun fetchAllInAppsFromConsole() {
        ...
        googleBilling.querySkuDetailsAsync(
            params.build(),
                ...
                override fun onSkuDetailsResponse(
                    ...
                    if (skuDetails != null) {
                        for (skuDetail in skuDetails) {
                            listAvailPurchases.add(skuDetail)
...
/*    fun fetchAIGpsNavPurchasedInAppsFromConsole() {
    ...
    fun fetchPurchasedInAppsFromConsole() {

        googleBilling.queryPurchasesAsync(BillingClient.SkuType.INAPP,object :PurchasesResponseListener{
                    ...
                    if (listPurchased.size > 0) {
                        for (singlePurchase in listPurchased) {
                            if (singlePurchase.purchaseState == Purchase.PurchaseState.PURCHASED) {
                                ...
                                billingPreferences.edit().putBoolean(singlePurchase.skus[0], true).apply()
                            } else {
                                billingPreferences.edit().putBoolean(singlePurchase.skus[0], false).apply()
    ...
    fun purchaseAdsPackage() {
        Log.d(TAG, "Going to purchase ads_purchase")
        if (listAvailPurchases.size > 0) {
            try {
                val flowParams = BillingFlowParams.newBuilder()
                    .setSkuDetails(listAvailPurchases[0])
                    .build()
                val responseCode =
                    googleBilling.launchBillingFlow(
    ...
    fun purchaseAIGpsNavPremiumModules() {
        ...
        if (listAvailPurchases.size > 0) {
            try {
                val flowParams = BillingFlowParams.newBuilder()
                    .setSkuDetails(listAvailPurchases[1])
                    .build()
                val responseCode =
                    googleBilling.launchBillingFlow(
                        activityContext as Activity,
    ...
    fun purchaseAIGpsNavBothAdsModules() {
        ...
        if (listAvailPurchases.size > 0) {
            try {
                val flowParams = BillingFlowParams.newBuilder()
                    .setSkuDetails(listAvailPurchases[2])
                    .build()
                val responseCode =
                    googleBilling.launchBillingFlow(
                        activityContext as Activity,
    ...
    private val purchaseAcknowledgedListener: AcknowledgePurchaseResponseListener =
        AcknowledgePurchaseResponseListener { p0 ->
            Log.d(TAG, "Success Acknowledged : ${p0.responseCode}  :${p0.debugMessage}")
            fetchPurchasedInAppsFromConsole()
        }
        ...
        if (purchase.purchaseState == Purchase.PurchaseState.PURCHASED) {
            if (!purchase.isAcknowledged) {
                ...
                val acknowledgeParamaters = AcknowledgePurchaseParams.newBuilder()
                    ...
                    .build()
                googleBilling.acknowledgePurchase(
